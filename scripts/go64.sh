...
#
#
# In order to disable DSP power off when the DSP is not in use (no client connected), set
#  the C64_PWROFF variable before running this script:
#
#  $ export C64_PWROFF=0
#
#  (note) when this is set to 0, the DSP will still be powered off when the kernel
#          module is unloaded.
#
#
...
if [ "x" != "x$C64_PWROFF" ]; then
  KMODARGS_PWROFF="pwroff=$C64_PWROFF"
else
  KMODARGS_PWROFF=
fi

/sbin/insmod ./c64.ko $KMODARGS $KMODARGS_PWRLOG $KMODARGS_PWROFF
